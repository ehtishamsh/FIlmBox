...
function Cards({ type, id, rating, imgUrl, title }) {
  return (
    <Link
      to={`${type}/${id}`}
      className="flex flex-col relative gap-5 p-2 rounded-xl h-full bg-[rgba(146,146,146,0.12)] w-11/12 max-xxsm:w-full max-xl:pb-2 backdrop-blur-sm"
    >
      ...
      <div className="overflow-hidden rounded-lg  w-full">
        <img
          ...
          alt=""
          className="object-cover bg-cover  w-full h-[25rem] hover:transform hover:scale-110 transition-all duration-500"
        />
      </div>
